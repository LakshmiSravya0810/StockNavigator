...
# Callback for displaying indicators ----updates
@app.callback(
...
def get_more(df):
    ...
    # Create subplots
    fig = make_subplots(rows=3, cols=1, shared_xaxes=True, vertical_spacing=0.02, subplot_titles=('EWA 20', 'RSI', 'Bollinger Bands'), row_width=[0.2, 0.2, 0.6])
    ...
    fig.add_trace(go.Scatter(x=df['Date'], y=df['Upper'], mode='lines', name='Upper Band', line=dict(color='rgba(255, 0, 0, 0.5)')), row=3, col=1)
    fig.add_trace(go.Scatter(x=df['Date'], y=df['Lower'], mode='lines', name='Lower Band', line=dict(color='rgba(0, 0, 255, 0.5)')), row=3, col=1)
